...
interface PieChartProps {
  ...
  /**
   * Pre-computed gradient fills keyed by series title.
   * When provided, slices (and legend items) use these colors instead of the
   * per-series palette. Computed once in PieChartPanel and shared here.
   */
  gradientFills?: Map<string, string>;
}
              ...
              <>
                {pie.arcs.map((arc) => {
                  ...
                  const fill = gradientFills
                    ? (gradientFills.get(arc.data.display.title ?? '') ?? arc.data.display.color ?? FALLBACK_COLOR)
                ...
                {showLabel &&
                  pie.arcs.map((arc) => {
                    ...
                    const fillForLabel = gradientFills?.get(arc.data.display.title ?? '');
 ...
 *
 * @returns A Map keyed by `FieldDisplay.display.title` → hex color string.
 */
export function computeGradientFills(items: FieldDisplay[], colorFrom: string, colorTo: string): Map<string, string> {
  ...
  for (let rank = 0; rank < n; rank++) {
    ...
    fills.set(item.display.title ?? '', tinycolor({ r, g, b }).toHexString());
  }
